...
//		GroupBy(collection.FieldPublishedAt).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
//
//	client.Collection.Query().
//		Select(collection.FieldPublishedAt).
//		Scan(ctx, &v)
//
...
func (cq *CollectionQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if ps := cq.order; len(ps) > 0 {
		_spec.Order = func(selector *sql.Selector) {
			for i := range ps {
				ps[i](selector, collection.ValidColumn)
			}
		}
	}
	return _spec
...
func (cq *CollectionQuery) sqlQuery(ctx context.Context) *sql.Selector {
	...
	selector := builder.Select(t1.Columns(collection.Columns...)...).From(t1)
	if cq.sql != nil {
		selector = cq.sql
		selector.Select(selector.Columns(collection.Columns...)...)
	}
	for _, p := range cq.predicates {
		p(selector)
	}
	for _, p := range cq.order {
		p(selector, collection.ValidColumn)
	}
	if offset := cq.offset; offset != nil {
...
func (cgb *CollectionGroupBy) sqlQuery() *sql.Selector {
	...
	for _, fn := range cgb.fns {
		columns = append(columns, fn(selector, collection.ValidColumn))
	}
...
func (cs *CollectionSelect) sqlQuery() sql.Querier {
	selector := cs.sql
